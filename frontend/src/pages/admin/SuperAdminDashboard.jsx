...
const StatCard = ({ icon, label, value, sub, highlight }) => (
  <div style={{
    background: highlight
      ? 'linear-gradient(135deg, #1a2e1a 0%, #141414 100%)'
      : 'linear-gradient(135deg, #1a1a1a 0%, #141414 100%)',
    border: `1px solid ${highlight ? '#22c55e30' : '#262626'}`,
    borderRadius: '16px',
    padding: '24px',
    flex: '1 1 200px',
    minWidth: '180px',
    ...
    <div style={{ color: highlight ? '#22c55e' : '#fafafa', fontSize: '28px', fontWeight: 700 }}>{value}</div>
...
const SuperAdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      setLoading(true);
      try {
        const { data } = await superadminAPI.getDashboardStats(period);
        setStats(data.data);
      } catch (err) {
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        <h1 style={{ color: '#fafafa', fontSize: '24px', fontWeight: 700 }}>Platform Dashboard</h1>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
        <h1 style={{ color: '#fafafa', fontSize: '24px', fontWeight: 700, margin: 0 }}>
          Platform Dashboard
        </h1>
      ...
      <div style={{
        ...
        <h3 style={{ color: '#fafafa', fontSize: '16px', fontWeight: 600, marginBottom: '8px' }}>
          Revenue Breakdown
        </h3>
        <div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap', marginTop: '16px' }}>
          ...
          <div>
            <div style={{ color: '#737373', fontSize: '13px' }}>Platform Commission ({stats.commissionRate}%)</div>
            <div style={{ color: '#22c55e', fontSize: '24px', fontWeight: 700 }}>
              ₹{(stats.platformCommission || 0).toLocaleString('en-IN')}
            </div>
          </div>
          <div>
            <div style={{ color: '#737373', fontSize: '13px' }}>Restaurant Net Payouts</div>
            <div style={{ color: '#f59e0b', fontSize: '24px', fontWeight: 700 }}>
              ₹{(stats.restaurantNetRevenue || 0).toLocaleString('en-IN')}
            </div>
          </div>
        </div>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #262626' }}>
                {['Restaurant', 'Orders', 'Gross Revenue', 'Commission (20%)', 'Net Payout'].map((h) => (
            ...
            <tbody>
              {stats.perRestaurantRevenue.map((r, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #1a1a1a' }}>
                  ...
                  <td style={{ padding: '12px', color: '#22c55e', fontWeight: 600 }}>₹{r.commission.toLocaleString('en-IN')}</td>
                  <td style={{ padding: '12px', color: '#f59e0b' }}>₹{r.netRevenue.toLocaleString('en-IN')}</td>
                </tr>
              ))}
            </tbody>
          </table>
